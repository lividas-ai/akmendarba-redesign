...
The complete public portfolio mirror lives in `public/assets/portfolio/`. Its 118 optimized WebP files, nine gallery categories, local slugs, captions, and exact source URLs are recorded in `src/client/granit-decor/data/portfolio-gallery.ts`. The smaller curated set below is also used for editorial page heroes and project-detail demonstrations.
...
- Earlier landscape and reduced-resolution exports are recorded in the source project history but intentionally excluded from the reusable template package.
...
- A rejected independent portrait generation did not reproduce the approved desktop orbit and is intentionally excluded from the reusable template package.
...
All 133 source and optimized URLs are recorded beside their exact public names in `src/client/granit-decor/data/materials.ts`. Local assets live in `public/assets/materials/`.
